...
void calc_cpu(float B, float kT, float QX, float QZ, float *E_avg, float *M_avg, float *E_var, float *M_var, bool *lattice);
...
int main(){
    ...
    float B = 0.0;
    float kT[3] = {2.0,3.5,5.0};
    float QX = -0.5;
    float QZ = -1.0;

    bool lattice[grid_size];
    
    char filename[20];
    sprintf(filename, "results.dat");
    FILE *fp;
    fp = fopen(filename, "w");
    fprintf(fp, "x\ty\tspin\tface\tT\tQY\tQZ\n");
    for(int pari=0;pari<3;pari++){
            printf("Performing calculation at B=%g, kT=%g, QX=%g, QZ=%g\n", B, kT[pari], QX, QZ);
            calc_cpu(B, kT[pari], QX, QZ, &Emean, &Mmean, &Evar, &Mvar, lattice);
            
            // print xy plane
            for(int i=0;i<grid_dim_x;i++){ //x coord
                for(int j=0;j<grid_dim_x;j++){ //y coord
                    fprintf(fp, "%i\t%i\t%i\txy\t%.1f\t%.1f\t%.1f\n", 
                            i, j, lattice[ i+j*grid_dim_x ],
                            kT[pari],QX,QZ);
                }
            }
            // print xz plane
            for(int i=0;i<grid_dim_x;i++){ //x coord
                for(int j=0;j<grid_dim_x;j++){ //z coord
                    fprintf(fp, "%i\t%i\t%i\txz\t%.1f\t%.1f\t%.1f\n", 
                            i, j, lattice[i+j*grid_dim_z],
                            kT[pari],QX,QZ);
                }
            }
            // print yz plane
            for(int i=0;i<grid_dim_x;i++){ //y coord
                for(int j=0;j<grid_dim_x;j++){ //z coord
                    fprintf(fp, "%i\t%i\t%i\tyz\t%.1f\t%.1f\t%.1f\n", 
                            i, j, lattice[i*grid_dim_x+j*grid_dim_z],
                            kT[pari],QX,QZ);
                }
            }
    } //end pari
...
void calc_cpu(float B, float kT, float QX, float QZ, float *E_avg_out, float *M_avg_out, float *E_var_out, float *M_var_out, bool *lattice){
    // Degbug things
    // Template:
    ...
    *M_var_out = magnet.var();
    cudaMemcpy( lattice, dev_lattice, grid_size*sizeof(bool), cudaMemcpyDeviceToHost);
