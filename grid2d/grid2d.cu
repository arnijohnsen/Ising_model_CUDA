...
void calc_cpu(float B, float kT, float Q, float *E_avg, float *M_avg, float *E_var, float *M_var, bool *lattice);
...
int main(){
    ...
    float B = 0.0;
    float Q [4]  = {-1.0,-0.5,0.5,1.0};
    float kT [4] = { 1.0, 1.7,2.27,4.0};
    ...
    FILE *fp;
    fp = fopen(filename, "w");
    fprintf(fp, "y\tx\tspin\tkT\tQ\n");
    for(int pari=0; pari<4;pari++){
        for(int parj=0;parj<4;parj++){
            printf("Performing calculation at B=%g, kT=%g, Q=%g\n", B, kT[pari], Q[parj]);
            calc_cpu(B, kT[pari], Q[parj], &Emean, &Mmean, &Evar, &Mvar, lattice);
            //fprintf(fp, "# %.6e\t%.6e\t%.6e\t%.6e\t%.6e\t%.6e\t%.6e\n", B, kT, Q, Emean, Mmean, Evar, Mvar);
            for (int i=0;i<print_size;i++){ //row
                for (int j=0;j<print_size;j++){ //column
                    fprintf(fp, "%i\t%i\t%i\t%.1f\t%.1f\n", i, j, lattice[i*grid_dim_x+j], kT[pari], Q[parj]);
                }
            }
        }
    }
...
void calc_cpu(float B, float kT, float Q, float *E_avg_out, float *M_avg_out, float *E_var_out, float *M_var_out, bool *lattice){
    // Degbug things
    // Template:
